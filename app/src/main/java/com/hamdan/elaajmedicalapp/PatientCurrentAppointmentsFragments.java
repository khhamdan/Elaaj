...
import com.hamdan.elaajmedicalapp.adapter.PatientCurrentAppointmentAdapter;
...
public class PatientCurrentAppointmentsFragments extends Fragment {


    IpAddress ipAddress;
    String url = "http://" + ipAddress.ip + ":8000/api/showPatientCurrentAppointments";
    ...
    String doctor_id, appointment_id, appointmentDate, appointmentTime, appointmentType, docProfilePic, doctorName, doctorExpertise;
    String userPatientId,patientName,patientEmail;;
    private RecyclerView recyclerView;
    private RecyclerViewInterface recyclerViewInterface;
    SwipeRefreshLayout swipeRefreshLayout;
    PatientCurrentAppointmentAdapter patientCurrentAppointmentAdapter;

    @Override
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        // Inflate the layout for this fragment
        ViewGroup root = (ViewGroup) inflater.inflate(R.layout.patient_current_appointment_fragment, container, false);

        user_detailsList = new ArrayList<>();
        doctor_detailsList = new ArrayList<>();
        appointmentsList = new ArrayList<>();
        recyclerView = root.findViewById(R.id.currentAppointmentRecyclerview);
        recyclerView.setHasFixedSize(true);
        recyclerView.setLayoutManager(new LinearLayoutManager(getActivity()));
        swipeRefreshLayout = root.findViewById(R.id.swipeRefreshCurrentAppointments);
        ...
        swipeRefreshLayout.setOnRefreshListener(new SwipeRefreshLayout.OnRefreshListener() {
            ...
            public void onRefresh() {
                getAppointmentList();
                getAppointmentList();
                patientCurrentAppointmentAdapter.notifyDataSetChanged();
            ...
            public void onResponse(String response) {
                ...
                try {
                    ...
                    for (int i = 0; i < appointmentsDetails.length(); i++) {
                        ...
                        String doctorStatus = object.getString("status");
                        ...
                        Appointments appointments = new Appointments(appointment_id, appointmentDate, appointmentTime, appointmentType,doctorStatus, userPatientId,user_details_id_of_doctor);
                        user_detailsList.add(user_details);
                        doctor_detailsList.add(doctor_details);
                        appointmentsList.add(appointments);
                        patientCurrentAppointmentAdapter =new PatientCurrentAppointmentAdapter(getActivity(), appointmentsList, user_detailsList, doctor_detailsList, new PatientCurrentAppointmentAdapter.ItemClickListener() {
                            @Override
                        ...
                        recyclerView.setAdapter(patientCurrentAppointmentAdapter);
                    }
                } catch (Exception e) {
                    e.printStackTrace();
                }
